import { Component, Inject, Injectable, Renderer2, RendererFactory2 } from '@angular/core';
...
*/
  public changeTheme(color) {
    ...
    if (color == "vh-theme") {
      localStorage.removeItem("vh_theme");
      return;
    }
    this.renderer.addClass(this.document.body, color);
    localStorage.setItem("vh_theme", color);
  }
  ...
  public activeAndroidBackButton(path: string, action?, priority: number = 1) {
    if (this.platform.platforms().includes("android") && (path ? this.router.url.includes(path) : true)) {
